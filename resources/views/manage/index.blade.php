        ...
        <div class="bg-blue-50 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            ...
            <ul class="space-y-2">
                @foreach ($posts as $post)
                    <li class="group flex items-center justify-between p-3 bg-white rounded-lg hover:bg-blue-100 transition-colors">
                        ...
                        <div class="flex items-center space-x-2 ml-3">
                            <a href="{{ route('posts.edit', $post) }}" class="text-blue-400 hover:text-blue-600 p-1 rounded-full hover:bg-blue-50">
                            ...
                            <form action="{{ route('posts.destroy', $post) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you sure you want to delete this post?')" class="text-red-400 hover:text-red-600 p-1 rounded-full hover:bg-red-50">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-green-50 p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-green-800 flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M3 4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2H3zm0 2h12v3H3V6zm0 5h6v3H3v-3zm0 5h6v2H3v-2zm8-5h4v2h-4v-2zm0 5h4v2h-4v-2z"/>
                    </svg>
                    Categories
                </h2>
            ...
            <ul class="space-y-2">
                @foreach ($categories as $category)
                    <li class="flex items-center justify-between p-3 bg-white rounded-lg hover:bg-green-100 transition-colors">
                        <span class="text-gray-700">{{ $category->name }}</span>
                        <div class="flex items-center space-x-2">
                            <button onclick="openEditCategoryModal('{{ $category->id }}', '{{ $category->name }}')" class="text-blue-400 hover:text-blue-600 p-1 rounded-full hover:bg-blue-50">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                </svg>
                            </button>
                            <form action="{{ route('manage.deleteCategory', $category) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you sure you want to delete this category?')" class="text-red-400 hover:text-red-600 p-1 rounded-full hover:bg-red-50">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </button>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span class="px-2 py-1 rounded-full {{ $user->role_id === 1 ? 'bg-purple-200 text-purple-800' : 'bg-gray-200 text-gray-600' }}">
                                    {{ $user->role->name }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-sm space-x-2">
                                <button onclick="openRoleModal('{{ $user->id }}', '{{ $user->role_id }}')" class="text-blue-400 hover:text-blue-600 p-1 rounded-full hover:bg-blue-50">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                    </svg>
                                </button>
                                <form action="{{ route('manage.deleteUser', $user) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this user?')" class="text-red-400 hover:text-red-600 p-1 rounded-full hover:bg-red-50">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </button>
...
function openRoleModal(userId, currentRoleId) {
    ...
    form.querySelector('[name="role_id"]').value = currentRoleId;
